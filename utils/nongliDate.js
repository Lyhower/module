import dateFormat from "./dateFormat";

const lunarInfo = new Array(0x04bd8, 0x04ae0, 0x0a570, 0x054d5, 0x0d260, 0x0d950, 0x16554, 0x056a0, 0x09ad0, 0x055d2,
	0x04ae0, 0x0a5b6, 0x0a4d0, 0x0d250, 0x1d255, 0x0b540, 0x0d6a0, 0x0ada2, 0x095b0, 0x14977, 0x04970, 0x0a4b0,
	0x0b4b5, 0x06a50, 0x06d40, 0x1ab54, 0x02b60, 0x09570, 0x052f2, 0x04970, 0x06566, 0x0d4a0, 0x0ea50, 0x06e95,
	0x05ad0, 0x02b60, 0x186e3, 0x092e0, 0x1c8d7, 0x0c950, 0x0d4a0, 0x1d8a6, 0x0b550, 0x056a0, 0x1a5b4, 0x025d0,
	0x092d0, 0x0d2b2, 0x0a950, 0x0b557, 0x06ca0, 0x0b550, 0x15355, 0x04da0, 0x0a5d0, 0x14573, 0x052d0, 0x0a9a8,
	0x0e950, 0x06aa0, 0x0aea6, 0x0ab50, 0x04b60, 0x0aae4, 0x0a570, 0x05260, 0x0f263, 0x0d950, 0x05b57, 0x056a0,
	0x096d0, 0x04dd5, 0x04ad0, 0x0a4d0, 0x0d4d4, 0x0d250, 0x0d558, 0x0b540, 0x0b5a0, 0x195a6, 0x095b0, 0x049b0,
	0x0a974, 0x0a4b0, 0x0b27a, 0x06a50, 0x06d40, 0x0af46, 0x0ab60, 0x09570, 0x04af5, 0x04970, 0x064b0, 0x074a3,
	0x0ea50, 0x06b58, 0x055c0, 0x0ab60, 0x096d5, 0x092e0, 0x0c960, 0x0d954, 0x0d4a0, 0x0da50, 0x07552, 0x056a0,
	0x0abb7, 0x025d0, 0x092d0, 0x0cab5, 0x0a950, 0x0b4a0, 0x0baa4, 0x0ad50, 0x055d9, 0x04ba0, 0x0a5b0, 0x15176,
	0x052b0, 0x0a930, 0x07954, 0x06aa0, 0x0ad50, 0x05b52, 0x04b60, 0x0a6e6, 0x0a4e0, 0x0d260, 0x0ea65, 0x0d530,
	0x05aa0, 0x076a3, 0x096d0, 0x04bd7, 0x04ad0, 0x0a4d0, 0x1d0b6, 0x0d250, 0x0d520, 0x0dd45, 0x0b5a0, 0x056d0,
	0x055b2, 0x049b0, 0x0a577, 0x0a4b0, 0x0aa50, 0x1b255, 0x06d20, 0x0ada0);
const Animals = new Array("鼠", "牛", "虎", "兔", "龙", "蛇", "马", "羊", "猴", "鸡", "狗", "猪");
const Gan = new Array("甲", "乙", "丙", "丁", "戊", "己", "庚", "辛", "壬", "癸");
const Zhi = new Array("子", "丑", "寅", "卯", "辰", "巳", "午", "未", "申", "酉", "戌", "亥"); //传入 offset 传回干支, 0=甲子

function cyclical(num) {
	return Gan[num % 10] + Zhi[num % 12];
}

//传回农历 y年的总天数


function lYearDays(y) {
	var i, sum = 348;
	for (i = 0x8000; i > 0x8; i >>= 1) sum += lunarInfo[y - 1900] & i ? 1 : 0;
	return sum + leapDays(y);
}
/* 
 传回农历 y年闰月的天数
*/


function leapDays(y) {
	if (leapMonth(y)) return lunarInfo[y - 1900] & 0x10000 ? 30 : 29;
	else return 0;
} //传回农历 y年闰哪个月 1-12 , 没闰传回 0


function leapMonth(y) {
	return lunarInfo[y - 1900] & 0xf;
} // 传回农历 y年m月的总天数


function monthDays(y, m) {
	return lunarInfo[y - 1900] & 0x10000 >> m ? 30 : 29;
} // 获取时间对应农历年份


function lunarYeay(date) {
	let baseDate = new Date("1901/02/19 00:00:00"); //1901年正月初一的阳历日期,不支持1901年以前的日期

	let offset = dateFormat.timestamp(baseDate); //1901年正月初一的时间戳

	let times = dateFormat.timestamp(date);
	let temp = offset;
	let year = 0;

	for (let i = 1901; i < 2050; i++) {
		if (times >= temp) {
			temp += lYearDays(i) * 86400;
			year = i;
			//console.log(dateFormat.date(temp * 1000)); //春节
		} else {
			break;
		}
	}

	temp -= lYearDays(year) * 86400; //console.log(dateFormat.date(temp * 1000)); //春节

	let months = [];

	for (let i = 1; i < 13; i++) {
		months.push({
			m: i,
			d: monthDays(year, i)
		});
	}

	let leap_Month = leapMonth(year);

	if (leap_Month > 0) {
		//存在闰月
		let leap_MonthDays = leapDays(year);
		months.splice(leap_Month, 0, {
			m: leap_Month,
			d: leap_MonthDays
		});
	}

	let month = 0;

	for (let i = 0; i < months.length; i++) {
		if (times >= temp) {
			temp += months[i].d * 86400;
			month = months[i].m;
		} else {
			break;
		}
	}

	;
	temp -= monthDays(year, month) * 86400;
	let day = parseInt((times - temp) / 86400) + 1;
	return {
		Y: cYear(year),
		M: cMonth(month),
		D: cDay(day)
	};
} //中文年


function cYear(y) {
	var s = "零,一,二,三,四,五,六,七,八,九,十".split(",");
	var syeay = y.toString().split('');
	var ss = [];
	syeay.forEach(e => {
		ss.push(s[parseInt(e)]);
	});
	return ss.join('');
} //中文月


function cMonth(m) {
	var s = "正,二,三,四,五,六,七,八,九,十,十一,十二".split(",");
	return s[m - 1];
} //中文日


function cDay(d) {
	var s = "初一,初二,初三,初四,初五,初六,初七,初八,初九,初十,十一,十二,十三,十四,十五,十六,十七,十八,十九,二十,廿一,廿二,廿三,廿四,廿五,廿六,廿七,廿八,廿九,三十".split(",");
	return s[d - 1];
}

module.exports = {
	//获取农历某月的最大天数
	monthDays(y, m) {
		return monthDays(y, m);
	},
	// 公历转农历
	conver(date) {
		return lunarYeay(date);
	},
	//获取农历年份
	lunarYeay(date) {
		return lunarYeay(date).Y;
	},
	//获取闰月，没有闰月返回0
	leapMonth(y) {
		return leapMonth(y);
	},
	//获取闰月天数，没有闰月返回0
	leapDays(y) {
		return leapDays(y);
	},
	//获取某年某月天数
	monthDays(y, m) {
		return monthDays(y, m);
	},
	//中文年
	cYear(y) {
		var s = "O,一,二,三,四,五,六,七,八,九,十".split(",");
		var syeay = y.toString().split('');
		var ss = [];
		syeay.forEach(e => {
			ss.push(s[parseInt(e)]);
		});
		return ss.join('');
	},
	//中文月
	cMonth(m) {
		return cMonth(m);
	},
	//中文日
	cDay(d) {
		return cDay(d);
	}
}; //console.log(monthDays(2020, 1));
