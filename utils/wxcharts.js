...
var util = {
	...
	isCollision(obj1, obj2) {
		...
		var flag = obj2.start.x > obj1.end.x || obj2.end.x < obj1.start.x || obj2.end.y > obj1.start.y || obj2.start
			.y < obj1.end.y;
		return !flag;
	}
};
...
function measureText(text) {
	var fontSize = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] :
	10; // wx canvas 未实现measureText方法, 此处自行实现
...
function isInExactChartArea(currentPoints, opts, config) {
	return currentPoints.x < opts.width - config.padding && currentPoints.x > config.padding + config.yAxisWidth +
		config.yAxisTitleWidth && currentPoints.y > config.padding && currentPoints.y < opts.height - config
		.legendHeight - config.xAxisHeight - config.padding;
}
...
function getRadarDataPoints(angleList, center, radius, series, opts) {
	...
	series.forEach(function(each) {
		...
		each.data.forEach(function(item, index) {
			...
			tmp.position = convertCoordinateOrigin(radius * tmp.proportion * process * Math.cos(tmp
				.angle), radius * tmp.proportion * process * Math.sin(tmp.angle), center);
...
function getDataPoints(data, minRange, maxRange, xAxisPoints, eachSpacing, opts, config) {
	...
	data.forEach(function(item, index) {
		if (item === null) {
			...
			height *= process;
			point.y = opts.height - config.xAxisHeight - config.legendHeight - Math.round(height) - config
				.padding;
			points.push(point);
		}
	});
...
function drawPieText(series, opts, config, context, radius, center) {
	...
	seriesConvert.forEach(function(item) {
		...
		var orginX3 = orginX1 >= 0 ? orginX1 + config.pieChartTextPadding : orginX1 - config
		.pieChartTextPadding;
		var orginY3 = orginY1;
...
function drawToolTip(textList, offset, opts, config, context) {
	...
	var toolTipHeight = 2 * config.toolTipPadding + textList.length * config
	.toolTipLineHeight; // if beyond the right border
	...
	series.forEach(function(eachSeries, seriesIndex) {
		...
		points.forEach(function(item, index) {
			if (item !== null) {
				...
				var height = opts.height - item.y - config.padding - config.xAxisHeight - config
					.legendHeight;
				...
				if (opts.extra.lineStyle === 'curve') {
					points.forEach(function(item, index) {
						if (index > 0) {
							...
							context.bezierCurveTo(ctrlPoint.ctrA.x, ctrlPoint.ctrA.y, ctrlPoint
								.ctrB.x, ctrlPoint.ctrB.y, item.x, item.y);
						}
					});
				} else {
					points.forEach(function(item, index) {
...
function drawXAxis(categories, opts, config, context) {
	...
	if (config._xAxisTextAngle_ === 0) {
		...
		categories.forEach(function(item, index) {
			...
			var _calRotateTranslate = calRotateTranslate(xAxisPoints[index] + eachSpacing / 2, startY + config
					.fontSize / 2 + 5, opts.height),
...
function drawLegend(series, opts, config, context) {
	...
	legendList.forEach(function(itemList, listIndex) {
		...
		var startY = opts.height - config.padding - config.legendHeight + listIndex * (config.fontSize +
			marginTop) + padding + marginTop;
		context.setFontSize(config.fontSize);
		itemList.forEach(function(item) {
			switch (opts.type) {
				case 'line':
...
function drawPieDataPoints(series, opts, config, context) {
	...
	var radius = Math.min(centerPosition.x - config.pieChartLinePadding - config.pieChartTextPadding - config
		._pieTextMaxLength_, centerPosition.y - config.pieChartLinePadding - config.pieChartTextPadding);
	...
	series.forEach(function(eachSeries) {
		...
		context.arc(centerPosition.x, centerPosition.y, radius, eachSeries._start_, eachSeries._start_ + 2 *
...
function drawRadarDataPoints(series, opts, config, context) {
	...
	var _loop = function _loop(i) {
		...
		coordinateAngle.forEach(function(angle, index) {
			var pos = convertCoordinateOrigin(radius / config.radarGridCount * i * Math.cos(angle), radius /
				config.radarGridCount * i * Math.sin(angle), centerPosition);
...
var Charts = function Charts(opts, canvasContext) {
	...
	this.context = canvasContext ? canvasContext : wx.createCanvasContext(opts
	.canvasId); // store calcuated chart data
	...
	if (touches && touches.length) {
		...
		if (this.opts.type === 'pie' || this.opts.type === 'ring') {
			...
			return findCurrentIndex({
					x: x,
					y: y
				}, this.chartData.xAxisPoints, this.opts, this.config, Math.abs(this.scrollOption
				.currentOffset));
		}
	}
	...
	if (this.opts.type === 'line' || this.opts.type === 'area') {
		...
		if (index > -1) {
			...
			if (seriesData.length !== 0) {
				var _getToolTipData = getToolTipData(seriesData, this.chartData.calPoints, index, this.opts
						.categories, option),
